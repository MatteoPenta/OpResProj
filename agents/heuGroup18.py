...
class HeuGroup18(Agent):
    ...
    def compute_VRP(self, deliveries_to_do, vehicles_dict, alns_N_max=None, alns_N_IwI=None):
        ...
        between the calls to this function. In particular, compute_VRP() is first called
        by the compute_delivery_to_crowdship() function: in this case, an initial solution is 
        generated by the constructiveVRP() function (Solomon heuristic), it is saved in self.sol
        and the flag init_sol_created is set to True. Then, when compute_VRP() is called again 
        from the main after the crowdshipping has ended, it will load the previously generated 
        initial solution from self.sol and use it. It will go again through the ALNS and the new
        best_sol will be generated.
        This procedure is done in order to call constructiveVRP() only once.

        Moreover, this procedure is executed ONLY when compute_VRP() is called from the crowdshipping
        function or from the main. When, instead, it is called from the learn_and_save() function, 
        a new sol is generated by constructiveVRP() every time.
        """
        ...
        if n_nodes_sol < self.env.n_deliveries - self.n_crowdshipped and best_sol_allnodes:
            ...
            # DEBUG
            print("[DEBUG] best_sol_allnodes chosen instead of best_sol")
    ...
    def constructiveVRP(self, deliveries_to_do, vehicles_dict):
        """
        Generate an initial solution using the Solomon heuristic.
        """
    ...
    def ALNS_VRP(self, sol, alns_N_max, alns_N_IwI):
        """
        Apply the ALNS heuristic on an already formed solution "sol".
        """
    ...
    def nodeIsFeasibleVRP(self, d, v_vol_left):
        """
        Determine whether inserting node "d" in a certain vehicle is feasible
        from the point of view of remaining capacity and status of the node, i.e. 
        if is is already present in the path of another vehicle.
    ...
    def nodeTimeFeasibleVRP(self, sol_k, prev_n_sol, d, next_n_sol):
        """
        Determine whether inserting node "d" in the vehicle characterized by "sol_k"
    ...
    def getC1(self, sol_k, prev_n_sol, d, next_n_sol):
        """
        Obtain the C1 cost given by inserting node d in the vehicle charactedized by sol_k
        between nodes indicated by prev_n_sol and next_n_sol
    ...
    def getC2(self, d, c1):
        """
        Obtain the C2 cost given by inserting node d in a certain vehicle.
        The info about the insertion in the specific vehicle is included in
        the C1 cost passed as argument.

        Parameters
            :param d : dictionary associated to the node d. Taken from the dictionary of dictionaries self.delivery 
            :type Dict

            :param c1 : C1 cost associated to the insertion of d in a vehicle
            :type Int
    ...
    def getC3(self, sol_k, best_pos_ve_k):
        """
        Obtain the C3 cost given by inserting node d in a the vehicle characterized by sol_k.
        ...
        Parameters
            ...
            :param best_pos_ve_k : [<best_pos_k of that vehicle>], 
                                    where best_pos_k = [<prev_n>,<next_n>,<c1>,<c2>,<node_id>]
            :type List
    ...
    def insertNode(self, sol_k, best_d_id, prev_n_sol, next_n_sol):
        ...
        Parameters
            ...
            :param best_d_id : id of the node which has to be inserted in the vehicle.
            :type Int
    ...
    def removeNode(self, sol_k, n_id, prev_n_sol, next_n_sol):
        """
        Remove node with id n_id from the vehicle characterized by sol_k. The position of 
        such node in the vehicle is given by its neighbours in the path, indicated by 
        prev_n_sol and next_n_sol
        ...
        Parameters
            ...
            :param d_id : id of the node which has to be removed from the vehicle.
            :type Int
    ...
    def restore_vehicles_order(self, sol):
        """
        Rearrange the elements of the list of dictionaries "sol", which contains the path,
        arrival times, waiting times and general info associated to every vehicle, so that 
        the resulting order is equal to the one specified in the input "setting.json" file.

        Parameters
            :param sol : list of dictionaries, where each element is the dictionary associated to 
                            a vehicle.
    ...
    def learn_and_save(self):
        """
        Several permutations of the available vehicles are tested ONLY DURING THE FIRST
        EXECUTION OF THIS FUNCTION (self.vehicles_dict still not defined, see first IF statement).
        The one which obtains the best objective function value is set as the definitive one.

        The following parameters are also trained:
            - self.volw
            - self.beta1_c3
            - self.alns_epsw
            - self.alns_p
